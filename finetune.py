...
class Processor:
    ...
    def test_all_checkpoints(self, epoch):
        """
        测试目录下所有的权重文件
        ...
        Args:
            ...
            device: 设备
        """
        # checkpoint_dir="/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_weight"
        checkpoint_dir="/media/zzf/ljn/wsx/output/weight_split_12/split_12"
        # 获取所有权重文件
        checkpoint_files = sorted(glob.glob(os.path.join(checkpoint_dir, "*.pth"))) + \
                           sorted(glob.glob(os.path.join(checkpoint_dir, "*.pt"))) + \
                           sorted(glob.glob(os.path.join(checkpoint_dir, "*.ckpt")))
        ...
        for checkpoint_path in checkpoint_files:
                ...
                # 根据权重文件结构加载模型参数
                # self.load_weights(self.encoder,checkpoint_path)
                self.load_text_weights(self.text_adapter,checkpoint_path)
                self.load_ske_weights(self.ske_adapter,checkpoint_path)
                self.test_epoch(epoch=epoch)
                # 进行测试
                self.log.info("Test Acc:{}".format(self.test_acc))
