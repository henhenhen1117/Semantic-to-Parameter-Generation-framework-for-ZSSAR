...
class Processor:
    ...
    def generate_optimization_instruction(self,error_report):
        ...
        return instructions
    def is_peocess_oriented(self,text):
        verbs=["take","lift","place","move","remove","raise"]
        body=["hand","arm","head","foot"]
        spatial=["to","onto","from","away"]
        score=sum(v in text for v in verbs)\
        +sum(b in text for b in body)\
        +sum(s in text for s in spatial)
        if score>=2:
            return True
        else:
            return False
...
# }
    ...
    def call_gpt4_optimizer(self,target_action, current_desc, confused_action, error_count):
        ...
        """
        prompt=f"""
        You are rewriting human action descriptions for skeleton-based action recognition.
        Rewrite the given action description by:
        1.Describing the physical motion as a sequence of concrete body movements.
        2.Focusing on observable actions rather than purposes or meanings.
        3.Explicitly mentioning interaction with objects or body parts when applicable.
        4.Avoiding abstract explanations or high-level definitions.
        Use a single concise sentence that emphasizes the motion process.
        ...
        #
        # Current Description for '{confused_action}':
        # "{current_desc}"
        #
        # Task:
        # Rewrite the description for '{confused_action}' to make it distinct from '{target_action}'.
        #
        # Requirements:
        # 1. Focus on skeleton motion sequence.
        # 2. Highlight what makes '{confused_action}' physically different from '{target_action}'.
...
#             return None
    ...
    def optimize_prompts_from_errors(self,error_report, database):
        ...
        for item in error_report:
            ...
            # 1. 获取当前的旧描述
            current_desc = database.get(confused, "Unknown action")
            ...
            if new_desc:
                ...
                if new_desc.lower().startswith(confused.lower()):
                    # 切掉动作名长度的字符
                    new_desc = new_desc[len(confused):].strip()

                # 3. 处理一种特殊情况：如果切完后开头还有引号或冒号（比如 ": involves..."），也去掉
                new_desc = new_desc.lstrip('":, ')

                # 4. 【强制格式化】：无论 LLM 返回什么，强行加上带引号的动作名
                final_desc = f'"{confused}" {new_desc}'

                # 5. 更新数据库
                database[confused] = final_desc
