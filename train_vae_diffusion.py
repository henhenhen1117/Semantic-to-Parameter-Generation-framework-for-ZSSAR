...
class Processor:
    ...
    def load_data(self, train_list, train_label, test_list, test_label, batch_size, language_path):
        ...
        self.embedding=np.load("./data/language/ntu60_des_embeddings.npy")
        # self.embedding=np.load("./data/learnable_context.npy")
        # self.embedding=np.load("prompt_embeddings.npy")
        ...
        # self.train_set = DataSet("/media/zzf/ljn/wsx/output/weight_split_12/enhanced_checkpoints", dim_per_token=768,
        #                   )
        self.train_set = DataSet("/media/zzf/ljn/wsx/output/weight_split_12/enhanced_checkpoints_all", dim_per_token=768,
                                 )
    ...
    def load_model(self, in_channels, hidden_channels, hidden_dim,
        ...
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/output/VAE_model/vae/best_vae_0.418508917093277.pt")
        self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/output/VAE_model/best_vae_bestloss:0.06.pt")#ACC:57.06
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/PGFA/PGFA-main/weight_log/split_12_VAE_train-epoch:200/best_vae_0.031790319830179214.pt")#test_vae
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/PGFA/PGFA-main/weight_log/split_12_VAE/best_vae_0.05107254162430763.pt")#49.06
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/PGFA/PGFA-main/weight_log/VAE/best_vae_0.05107254162430763.pt")#split_12_acc:71 +mappin_acc:85
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_vae/lr_vae/best_vae_0.046066880226135254.pt")
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/output/learnable_vae/lr_vae/best_vae_0.038198091089725494.pt")
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/PGFA/PGFA-main/weight_log/best_vae.pt")
        # self.load_vae_weights(self.vae,'/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_vae/best_vae_0.06984451413154602.pt')
        # self.load_vae_weights(self.vae,"/media/zzf/ljn/wsx/PGFA/PGFA-main/weight_log/split_5_VAE/best_vae_0.038198091089725494.pt")
        # self.load_transfer_weight(self.mapper,"/media/zzf/ljn/wsx/output/mapper/mapper_best_mappin2.pt")
        # self.load_transfer_weight(self.mapper,"/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_vae/mapper_best.pt")
        self.load_transfer_weight(self.mapper,"weight_log/mapper_best.pt")
    ...
    # def train_epoch(self, epoch, lr, loss_mode, step, loss_type, alpha, beta, m, fix_encoder):
        ...
        unseen_classes=[3, 7, 13, 19, 22, 28, 34, 41, 47, 52, 56, 59]
    ...
    # if this_steps % 60 == 0:
        ...
        self.generate("/media/zzf/ljn/wsx/output/weight_split_12/save_param/generated_model.pth", need_test=True,
                  condition=unseen_classes, epoch=epoch)
        with open("./train_Difffusion_new_loss.txt", 'a') as train_loss:
            train_loss.write(str(self.dim_loss) + '\n')
    ...
    def train_vae(self, epoch):
            ...
            if self.dim_loss < self.best_loss:
                ...
                self.best_loss = self.dim_loss
                self.save_vae_model('weight_log/split_12_VAE/best_vae_{}.pt'.format(self.best_loss))
                this_steps = 0
                train_loss = 0
            # if batch_idx >= 500:
            #     break
    ...
    def find_top_confusions(self,all_preds, all_labels, class_names_list, top_k=5):
        """
        all_preds: 预测的类别ID列表
    ...
    def test_epoch_prediction(self,prediction, unseen_label, epoch, DA, support_factor):
        ...
        class_names = ["Drink water", "Eat meal/snack", "Brushing teeth", "Brushing hair", "Drop", "“Pickup”", "Throw",
                       ...
                       "Jumping up", "Make a phone call/Answer phone", "Playing with phone/tablet",
                       "Typing on a keyboard", "Pointing to something with finger", "Taking a selfie",
                       "Checking time (from watch)", "Rubbing two hands together", "Nod headbow", "Shake head",
                       "Wiping face", "Salute", "Putting the palms together", "Crossing hands in front (saying stop)",
                       "Sneeze/cough", "Staggering", "Falling", "Touching head (headache)",
                       "Touching chest (stomachache/heart pain)", "Touching back (backache)",
                       "Touching neck (neckache)", "Nausea or vomiting",
                       "Using a fan (with hand or paper) to feel warm", "Punching/Slapping other person",
                       "Kicking other person", "Pushing other person", "Patting on back of other person",
                       "Pointing finger at the other person", "Hugging other person",
                       "Giving something to other person", "Touching other person's pocket", "Handshaking",
        ...
        feat_list = []
        old_pred_list = []
        self.weight=self.load_text_weights(self.text_adapter,"/media/zzf/ljn/wsx/output/weight_split_12/save_param/generated_model.pth")
        ...
        for data, label in tqdm(loader):
            ...
            data = data.type(torch.FloatTensor).cuda()
            label = label.type(torch.LongTensor).cuda()
            unseen_language = self.embedding[unseen_label].cuda()
        ...
        #
        # top_errors = self.find_top_confusions(all_preds, all_labels, class_names)
