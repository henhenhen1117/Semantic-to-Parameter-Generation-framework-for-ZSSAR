...
class Processor:
    ...
    def load_data(self, train_list, train_label, test_list, test_label, batch_size, language_path):
        ...
        # self.train_set = DataSet("/media/zzf/ljn/wsx/PGFA/PGFA-main/output/enhanced_learnable_checkpoints", dim_per_token=768,
        #                   )
        self.train_set = DataSet("/media/zzf/ljn/wsx/output/weight_split_12/enhanced_checkpoints",
                                 dim_per_token=768,
                                 )
    ...
    def load_text_weights_map(self, model=None, pretrained_dict=None):
        pretrained_dict = pretrained_dict
        model.load_state_dict(pretrained_dict,strict=False)

    def load_ske_weights_map(self, model=None, pretrained_dict=None):
        pretrained_dict = pretrained_dict
        model.load_state_dict(pretrained_dict,strict=False)
    ...
    def load_diffusion_weight(self, model=None, weight_path=None):
         pretrained_dict = torch.load(weight_path)
         # model.load_state_dict(pretrained_dict,strict=False)
         model.load_state_dict(pretrained_dict['diffusion'])
    ...
    def load_model(self, in_channels, hidden_channels, hidden_dim,
        ...
        # self.mapper=Mapper(768,256,rank=512, map_bias=True).cuda(device=0)
        # self.mapper = Mapper().cuda(device=1)
        # self.mapper=Mapper(768,256).cuda()
        ...
        # if fix_encoder or finetune:                                           `34`r   F
        ...
        self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/output/weight_split_12/best_vae.pt")
        # self.load_vae_weights(self.vae, "/media/zzf/ljn/wsx/PGFA/PGFA-main/output/VAE_model/vae/best_vae_0.16012847423553467.pt")
        # self.load_transfer_weight(self.mapper,"./output/mapper/mapper_best.pt")
        # self.load_transfer_weight(self.mapper,"/media/zzf/ljn/wsx/PGFA/PGFA-main/output/mapper/mapper_best_mappin2.pt")
        # self.load_transfer_weight(self.mapper,"/media/zzf/ljn/wsx/PGFA/PGFA-main/output/mapper/mapper_best_test_acc:76.92_add_acc:84.96.pt")
        ...
        # self.load_vae_weights(self.vae,'/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_vae/best_vae_0.06984451413154602.pt') #acc:88.20
        # self.load_vae_weights(self.vae,'./output/learnable_vae/best_vae.pt')#acc:85.28
        # self.load_vae_weights(self.vae,'/media/zzf/ljn/wsx/PGFA/PGFA-main/output/VAE_model/vae/best_vae_0.2218574434518814.pt')#acc:83.97
        # self.load_vae_weights(self.vae,'./output/learnable_vae/best_vae.pt')#acc:83.97
        # self.load_vae_weights(self.vae,'/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_vae/lr_vae/best_vae_0.23818571865558624.pt')#epoch [62] gets the best acc: 0.8482481241226196
        # self.load_vae_weights(self.vae,'/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_vae/best_vae_0.2148275524377823.pt')#epoch [62] gets the best acc: 0.8482481241226196
        # self.load_diffusion_weight(self.diffusion,'/media/zzf/ljn/wsx/PGFA/PGFA-main/output/VAE_model/diffusion/best_diffusion_acc_0.8538351655006409.pt')
    ...
    def generate(self,save_path=None, need_test=True,condition=None,epoch=None):
        ...
        diction=self.train_set.save_params(prediction, save_path=save_path)
    ...
    # def train_epoch(self, epoch, lr, loss_mode, step, loss_type, alpha, beta, m, fix_encoder):
        ...
        # for (param,_) in tqdm(self.train_loader):
            ...
            self.full_language = torch.Tensor(self.full_language)
            condition = self.full_language[seen_class].cuda(device=1)
            with torch.no_grad():
    ...
    def train_mappin(self, epoch):
        self.mapper.train()
        loss_list = []
        train_loss = 0
        this_steps = 0
        print("==> start training mappin..")

        unseen_classes=[3, 7, 13, 19, 22, 28, 34, 41, 47, 52, 56, 59]
        for i in 30:


            text_diction=self.generate("/media/zzf/ljn/wsx/PGFA/PGFA-main/output/save_param/generated_model.pth", need_test=True,
                  condition=unseen_classes, epoch=epoch)
            ske_mappin=self.mapper(text_diction['adapter.weight'],text_diction['adapter.bias'])
            self.load_ske_weights_map(text_diction)
            self.load_ske_weights_map(ske_mappin)
            self.test_epoch
    ...
    def train_vae(self, epoch):
            ...
            if self.dim_loss < self.best_loss:
                ...
                self.best_loss = self.dim_loss
                self.save_vae_model('weight/best_vae.pt')
                this_steps = 0
                train_loss = 0
            # if batch_idx >= 500:
            #     break
    ...
    def save_npy(self,skeleton_list,language_list):
        final_skeleton_features = np.concatenate(skeleton_list, axis=0)
        final_text_features = np.concatenate(language_list, axis=0)

        print(f"骨架特征形状: {final_skeleton_features.shape}")
        print(f"文本特征形状: {final_text_features.shape}")

        # 保存为 .npy 文件
        np.save(os.path.join('./output/dataset/', 'skeleton_feats.npy'), final_skeleton_features)
        np.save(os.path.join('./output/dataset/', 'text_feats.npy'), final_text_features)

        print(f"特征已保存至 {'./output/dataset/'}")
    @ex.capture
    def test_epoch_prediction(self,prediction, unseen_label, epoch, DA, support_factor):
        ...
        # self.ske_bias=self.mapper(self.weight['adapter.weight'],self.weight['adapter.bias'])
        ...
        for data, label in tqdm(loader):
            ...
            all_skeleton_features.append(feature)
            all_text_features.append(unseen_language)
        ...
        if acc > self.best_acc:
            ...
            # self.save_npy(all_skeleton_features,all_text_features)
