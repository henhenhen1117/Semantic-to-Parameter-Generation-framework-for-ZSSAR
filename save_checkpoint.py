...
class Processor:
    ...
    def layernorm(self, feature):
        ...
        return out
#added
    def save_ske_checkpoint(self,model, epoch, acc):
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        last_two_norms = ['adapter.weight', 'adapter.bias',
        ...
        torch.save(save_state,
                   f"checkpoint/{str(epoch).zfill(4)}_transfer_acc{acc:.4f}.pth")
        # print(f"Saved: checkpoint/{str(epoch).zfill(4)}_acc{acc:.4f}.pth")
        self.log.info(f"Saved: checkpoint/{str(epoch).zfill(4)}_transfer_acc{acc:.4f}.pth")

    def save_text_checkpoint(self, model, epoch, acc):
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        last_two_norms = ['adapter.weight', 'adapter.bias',
                          # 'model.layer4.1.bn2.weight', 'model.layer4.1.bn2.bias',
...
#added
    ...
    def load_model(self, in_channels, hidden_channels, hidden_dim,
        ...
        # self.prompt_learner = PromptLearner(self.class_description, self.bert, self.tokenizer).cuda(device=1)

        # self.adapter = Linear(256,768).cuda(device=1)
        self.ske_adapter = Linear(256, 512).cuda(device=1)
        ...
        self.logit_scale = self.ske_adapter.get_logit_scale().cuda()
        # self.logit_scale = self.adapter.get_logit_scale_v2()
        # self.logit_scale_v2 = self.adapter.get_logit_scale_v2()

        # if fix_encoder or finetune:
        #     self.load_weights(self.encoder, weight_path)
        # weight_paths='/media/zzf/ljn/wsx/PGFA/PGFA-main/output/weight/split_1_encoder_learnable_0.8235795497894287.pt'
        self.load_weights(self.encoder, weight_path)
        # self.load_ske_weights(self.ske_adapter,weight_path)
        # self.load_text_weights(self.text_adapter,weight_path)
        # self.load_prmpt_weights(self.prompt_learner,
        #                         "/media/zzf/ljn/wsx/PGFA/PGFA-main/output/prompt_learner/prmpt_learner_0.8490530252456665")
    ...
    def load_optim(self, lr, epoch_num, weight_decay):
        ...
        #     self.optimizer, T_max=300, eta_min=lr
        # )
    ...
    @ex.capture
    def train_epoch(self, epoch, lr, loss_mode, step, loss_type, alpha, beta, m, fix_encoder=False):
        ...
        for data, label in tqdm(loader):  # data shape:128,3,50,25,2;N,C,T,V,M
            data = data.type(torch.FloatTensor).cuda(device=1)
            # self.learnable_context=self.prompt_learner().cuda()
            ...
            # print(label) # int
            seen_language = self.full_language[label].cuda()  # 128, 768
            # seen_language = self.learnable_context[label]  # 128, 768
            # print(seen_language.shape)
            text_feat=seen_language

            feat = self.encoder(data)# stgcn: skeleton feature extraction 128,256

            if fix_encoder:
                feat = feat.detach()
            # skleton_feat = self.adapter(feat).cuda()# adapter project 256,768   shape:128,768
            skleton_feat = self.ske_adapter(feat).cuda()# adapter project 256,768   shape:128,768
            text_feat=self.text_adapter(seen_language)
            if loss_type == "kl":
                logits_per_skl, logits_per_text = create_logits(skleton_feat, text_feat, self.logit_scale, exp=True)
    ...
    def test_epoch(self, unseen_label, epoch, DA, support_factor):
        ...
        for data, label in tqdm(loader):
            ...
            feature = self.ske_adapter(feature).cuda()
            # feature = self.adapter(feature).cuda()
            unseen_language=self.text_adapter(unseen_language).cuda()
            if DA:
        ...
        acc = acc_list.mean()
        # self.save_ske_checkpoint(self.ske_adapter,epoch,acc)
        # self.save_text_checkpoint(self.text_adapter,epoch,acc)
