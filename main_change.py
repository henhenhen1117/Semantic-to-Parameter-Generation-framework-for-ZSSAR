...
from module.adapter import Adapter, Ske_Linear,Text_Linear,Linear
...
class Processor:
    ...
    def load_weights(self, model=None, weight_path=None):
        ...
        gcn_dict=pretrained_dict['encoder']
        model.load_state_dict(gcn_dict)
        # print(model)
    ...
    def load_model(self,in_channels,hidden_channels,hidden_dim,
        ...
        # self.ske_adapter = Ske_Linear().cuda()
        # self.text_adapter=Text_Linear().cuda()
    ...
    def train_epoch(self, epoch, lr, loss_mode, step, loss_type, alpha, beta, m, fix_encoder):
        ...
        for data, label in tqdm(loader):#data shape:128,3,50,25,2;N,C,T,V,M
            ...
            if fix_encoder:
                feat = feat.detach()
            skleton_feat = self.adapter(feat)#adapter project 256,768   shape:128,768
        ...
        #     print(f"初始化最佳loss为: {self.best_loss}")
        #
        #     # 检查当前epoch的loss是否更好
        ...
        #     # 保存最佳模型
        #     self.save_model()
        # #     print(f"发现新的最佳模型! Epoch: {epoch}, Loss: {epoch_loss:.6f}")
        # # else:
        # #     print(f"当前Epoch {epoch} Loss: {epoch_loss:.6f}, 最佳Loss: {self.best_loss:.6f}")
    ...
    def test_epoch(self, unseen_label, epoch, DA, support_factor):
        ...
        for data, label in tqdm(loader):
            ...
            # unseen_text_feat=self.text_adapter(unseen_language)
            if DA:
...
class SotaProcessor:
    ...
    def optimize(self, epoch_num, DA): # print -> log.info
        ...
        for epoch in range(epoch_num):
            ...
            self.log.info("epoch [{}] train loss: {}".format(epoch,self.dim_loss))
            # self.log.info("epoch [{}] test acc: {}".format(epoch,self.test_acc))
            self.log.info("epoch [{}] gets the best loss: {}".format(self.best_epoch,self.best_loss))
            if DA:
                self.log.info("epoch [{}] DA test acc: {}".format(epoch,self.test_aug_acc))
                self.log.info("epoch [{}] gets the best DA acc: {}".format(self.best_aug_epoch,self.best_aug_acc))
