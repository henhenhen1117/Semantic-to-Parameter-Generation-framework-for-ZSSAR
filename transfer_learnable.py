...
class Processor:
    ...
    def load_data(self, train_list, train_label, test_list, test_label, batch_size, language_path):
        ...
        self.dataset['train']=BaseTransferDataset("/media/zzf/ljn/wsx/PGFA/PGFA-main/output/weight/checkpoint_ori",normalize=True)
        # self.dataset['train']=BaseTransferDataset("/media/zzf/ljn/wsx/PGFA/PGFA-main/output/learnable_weight",normalize=True)
        ...
        self.w_ske_mean, self.w_ske_std, self.b_ske_mean, self.b_ske_std, self.w_text_mean, self.w_text_std, self.b_text_mean, self.b_text_std=self.dataset['train']._compute_stats()
    ...
    def layernorm(self, feature):
        ...
        return out
    def load_text_weights(self, model=None, pretrained_dict=None):
    ...
    def load_model(self,in_channels,hidden_channels,hidden_dim,
        ...
        # self.mapper=LinearInMapper(in_text=self.in_text,in_ske=self.in_ske).cuda()
    ...
    # self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(setransfer.pylf.optimizer, 100)
    ...
    def optimize(self, epoch_num, DA): # print -> log.info
        ...
        for epoch in range(epoch_num):
            ...
            self.log.info("epoch [{}] gets the best acc: {}".format(self.best_epoch_acc,self.best_acc))
            if DA:
    ...
    def train_epoch(self, epoch, lr, loss_mode, step, loss_type, alpha, beta, m, fix_encoder):
        ...
        for W_ske, b_ske, W_text, b_text in tqdm(loader):#data shape:128,3,50,25,2;N,C,T,V,M
            ...
            W_ske=W_pred.squeeze(0)*self.w_ske_std+self.w_ske_mean
            W_text=W_text.squeeze(0)*self.w_text_std+self.w_text_mean
            b_ske=b_pred.squeeze(0)*self.b_ske_std+self.b_ske_mean
            b_text=b_text.squeeze(0)*self.b_text_std+self.b_text_mean
            text_weight = collections.OrderedDict()
            ske_weight=collections.OrderedDict()
            text_weight['adapter.weight'] = W_text
            text_weight['adapter.bias'] = b_text
            ske_weight['adapter.weight'] = W_ske
            ske_weight['adapter.bias'] = b_ske
            # print(f"  W_ske: {W_ske.shape}")  # 应该是 [1, 512, 256] 或类似
            # print(f"  W_text: {W_text.shape}")  # 应该是 [1, 512, 768] 或类似
            self.load_text_weights(self.text_adapter,text_weight)
            self.load_ske_weights(self.ske_adapter,ske_weight)
    ...
    def test_epoch(self, epoch):
        ...
        if acc > self.best_acc:
            ...
            self.save_model()
            self.log.info(" test acc: {}".format(epoch,self.test_acc))
            # self.log.info("epoch [{}] gets the best acc: {}".format(self.best_epoch,self.best_acc))
    ...
    def save_model(self, save_path):
        ...
        torch.save(
            ...
            }, './output/mapper/mapper_best.pt')
    def save_loss_model(self):
        # os.makedirs(os.path.dirname(save_path), exist_ok=True)
        torch.save(
            {
            # 'encoder':self.encoder.state_dict(),
            ...
            # 'bias': self.bias_mapper.state_dict()
            # }, './output/mapper/mapper_epoch{}_loss:{}.pt'.format(self.best_epoch,self.best_loss))
            }, './output/mapper/mapper_best_loss.pt')
